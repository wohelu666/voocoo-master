...
public class DesiccantActivity extends AbsBaseActivity {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_buy_filter:
                Intent intent = new Intent(DesiccantActivity.this, WebActivity.class);
                intent.putExtra("url", "https://shop91447720.m.youzan.com/wscgoods/detail/26wy0ulpo36pc60");
                startActivity(intent);
                break;
            case R.id.tv_refresh:
                showLoading();
                HttpManage.getInstance().refreshDesiccan(device.getDeviceId(), new HttpManage.ResultCallback<String>() {
                    @Override
                    ...
                    public void onSuccess(int code, String response) {
                        ...
                        if (result.getCode() == 200) {
                            tvFilterDay.setText(result.getData() + "天");
                        }
                    }
                });
                break;
    ...
    private void initData() {
        HttpManage.getInstance().devDetail(device.getDeviceId(), new HttpManage.ResultCallback<String>() {
            ...
            public void onSuccess(int code, String response) {
                ...
                if (result.getCode() == 200) {
                    tvFilterSurplus.setText((int) ((result.getData().desiccanSurplusDays / 30f) * 100)+"%");
                    tvFilterDay.setText(result.getData().desiccanSurplusDays+ "天");
                }
            }
        });
    }
    ...
    private void initView() {
        ...
        tvTitle.setText(getString(R.string.text_left_desiccant));
    }

    @Override
    protected int getLayoutId() {
        return R.layout.activity_desiccant;
    }
