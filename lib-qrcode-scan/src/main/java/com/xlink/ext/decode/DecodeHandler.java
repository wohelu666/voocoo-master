...
final class DecodeHandler extends Handler {
    ...
    DecodeHandler(CaptureBaseActivity activity) {
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        String result = manager.decode(rotatedData, width, height, true, activity.getScanAreaPointX(), activity.getScanAreaPointY(), activity.getCropWidth(),
                activity.getCropHeight());
        Timber.v("---Zbar---DecodeHandler---decode---result==" + result);
        if (result != null) {
            if (activity.isNeedCapture()) {
                // 生成bitmap
                PlanarYUVLuminanceSource source = new PlanarYUVLuminanceSource(rotatedData, width, height, activity.getScanAreaPointX(), activity.getScanAreaPointY(),
